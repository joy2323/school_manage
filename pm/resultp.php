...
if(isset($_POST['sr'])){
	...
	//form validation
	if(!$sr || !$pn){
		$msg = "You must fill all form data. <a href='../result.php'>Try again</a>";
	}else{
		...
		if($count == 0){
			$msg = "That card was either used UP or the serial or pin is incorrect. <a href='../result.php'>Try again</a>";
		}else{
	...
	if ($used==0) {
		$sql = "UPDATE pins SET is_used='$use', by_who='$emal', date_used=now() WHERE id='$pid'";
			$quer = mysqli_query($con, $sql);
			
			if($quer){
				//redirect the user to the signup page
				header("location: ../../index.php?parents/marks/$en ");
					exit();
				}else{
				
			$msg = "That card was either used or the serial or pin is incorrect. <a href='../result.php'>Try again</a>";
			}
			
	}elseif ($used>0 && $emal=="$dbemail" ) {
