...
if(isset($_POST['sr'])){
	...
	if(!$sr || !$pn){
		...
		$sql = "SELECT * FROM pins WHERE serial='$sr' AND pin='$pn' AND is_used<1000";
	...
	if ($used==0) {
		$sql = "UPDATE pins SET is_used='$use', by_who='$ad', date_used=now() WHERE id='$pid'";
			$quer = mysqli_query($con, $sql);
			
			if($quer){
				//redirect the user to the signup page
				header("location: ../../index.php?admin/student_marksheet/$en  ");
					exit();
				}else{
