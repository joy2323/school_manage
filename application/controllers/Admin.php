...
class Admin extends CI_Controller
{
    ...
    function generatePin(){
      $digits = mt_rand(100, 99999);

      while (strlen($digits) < 5) {
        $digits .= 0;
      }

      $alpha = $this->incrementalHash();

      return $digits.$alpha;
    }

    /***ADMIN CREATE PIN PAGE***/
    function create_pin()
    {
        if ($this->session->userdata('admin_login') != 1)
            redirect(base_url(), 'refresh');
        $page_data['page_name']  = 'create_pin';
        $page_data['page_title'] = 'Create Result Pin';
        $this->load->view('backend/index', $page_data);
    }

    /***STUDENTS PASSWORD PAGE***/
    function students_password($param1 = "", $param2 = "")
    {
        if ($this->session->userdata('admin_login') != 1)
            redirect(base_url(), 'refresh');
        ...
        if($param2 == "true"){
          ...
          foreach($students as $row){
            $data['password'] = preg_replace('/\s+/', '', (mt_rand(10,999).strtolower($row['name'])) );
        ...
        $page_data['page_name']  = 'students_password';
        $page_data['page_title'] = 'Manage Students Password';
        $page_data['class_id'] = $param1;
        $this->load->view('backend/index', $page_data);
    }

    /***TEACHERS PASSWORD PAGE***/
    function teachers_password($param1 = "")
    {
        if ($this->session->userdata('admin_login') != 1)
            redirect(base_url(), 'refresh');
        ...
        if($param1 == "true"){
          ...
          foreach($teachers as $row){
            $data['password'] = preg_replace('/\s+/', '', (mt_rand(10,99).strtolower($row['name'])) );
        ...
        $page_data['page_name']  = 'teachers_password';
        $page_data['page_title'] = 'Manage Teachers Password';
        $this->load->view('backend/index', $page_data);
    }

    /***PARENTS PASSWORD PAGE***/
    function parents_password($param1 = "")
    {
        if ($this->session->userdata('admin_login') != 1)
            redirect(base_url(), 'refresh');
        ...
        if($param1 == "true"){
          ...
          foreach($parents as $row){
            $data['password'] = preg_replace('/\s+/', '', (strtolower($row['name']).mt_rand(10,99)) );
        ...
        $page_data['page_name']  = 'parents_password';
        $page_data['page_title'] = 'Manage Parents Password';
        $this->load->view('backend/index', $page_data);
    }
    ...
    function create_pins(){
      ...
      $data['sub_class']        = $this->input->post('section_id');

      $students = $this->db->get_where('student' , array(
          'class_id' => $data['class'],
          'section_id' => $data['sub_class']
      ))->result_array();
      ...
      foreach ($students as $row) {
          ...
          $array = array('student_id' => $student_id, 'session' => $data['session'], 'term' => $data['term'], 'class' => $data['class'], 'sub_class' => $data['sub_class']);
          $result = $this->db->get_where('result_pins' , $array)->result_array();

          if (count($result) < 1) {
            $data['student_id'] = $student_id;
            $data['pin'] = $this->generatePin();

            $this->db->insert('result_pins', $data);
          }
      }
      $this->result_pins($data['session'], $data['term'], $data['class'], $data['sub_class']);
    }


    /***ADMIN REDIRECT TO VIEW PINS PAGE***/
    function view_pin()
    {
        if ($this->session->userdata('admin_login') != 1)
            redirect(base_url(), 'refresh');
        $page_data['page_name']  = 'view_pin';
        $page_data['page_title'] = 'View Result Pins';
        $this->load->view('backend/index', $page_data);
    }

    function result_pins($session = '', $term = "", $class_id = '', $section_id = '')
  	{
  		if ($this->session->userdata('admin_login') != 1)
              redirect('login', 'refresh');

      if ( ($this->input->post('term') !== null) && ($this->input->post('session') !== null) && ($this->input->post('class_id') !== null) && ($this->input->post('section_id') !== null) ) {
      ...
      $result = $this->db->get_where('class', array('class_id' => $class_id ))->result_array();
      foreach ($result as $key) {
        $class_name = $key['name'];
        break;
      }

      $result = $this->db->get_where('section', array('class_id' => $class_id, 'section_id' => $section_id ))->result_array();
      foreach ($result as $key) {
        $sub_class_name = $key['name'];
        break;
      }
      ...
      elseif ($term == 3) {
        ...
        $t = $term;
  		...
  		$page_data['page_name']  	= 'result_pins';
  		$page_data['page_title'] 	= "Result pins for $class_name $sub_class_name, $t term $session academic session";
    ...
    function noticeboard($param1 = '', $param2 = '', $param3 = '')
    {
        ...
        if ($param1 == 'create') {
            ...
            $data['create_timestamp'] = strtotime(date('Y/m/d'));
