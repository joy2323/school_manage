...
class Student extends CI_Controller
{
    ...
    function  result_form()
    {
        if ($this->session->userdata('student_login') != 1)
            redirect(base_url(), 'refresh');
        $page_data['page_name']  = 'result_form';
        $page_data['page_title'] = "Result Checker";
        $this->load->view('backend/index', $page_data);
    }
    ...
    function view_result() {
        ...
        $result = $this->db->get_where('result_pins' , array('pin' => $pin))->row();

        if ($result) {
          $page_data['session'] = $result->session;
          $page_data['term'] = $result->term;
          $page_data['class_id'] = $result->class;
          $page_data['section_id'] = $result->sub_class;
          $page_data['student_id'] = $result->student_id;

          $class_info = $this->db->get_where('class', array('class_id', $page_data['class_id']))->row();
          $class_name = $class_info->name;

          $section_info = $this->db->get_where('section', array('section_id', $page_data['section_id']))->row();
          $section_name = $section_info->name;

          if ($page_data['term'] == 1) {
            $t = "1st";
          }
          elseif ($page_data['term'] == 2) {
            $t = "2nd";
          }
          elseif ($page_data['term'] == 3) {
            $t = "3rd";
          }
          else {
            $t = $term;
          ...
          $page_data['page_title'] = "$class_name $section_name $t term result";
        }
        else{
          $page_data['not_found'] = false;
        }
        $page_data['page_title'] =   "View result page";
