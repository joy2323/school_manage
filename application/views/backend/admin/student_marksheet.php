...
<form action="#" class="form-inline" method="post" <?php echo $hide; ?>>

	<label for="term">Please select the term:</label>
	<select name="choose_term" id="term" class="form-control">
	...
	<label for="session">Please select the session:</label>
	  <select name="choose_session" id="session" class="form-control">
...
<?php
																...
																if( $form>3){
																		$result='<center>	<table width="95%" border="3" cellspacing="1" cellpadding="2" class="table table-hover table-responsive">
...
$traits1=array("Punctuality","Class_Attendance","Attentiveness","Carrying_Out_Assignment","Participation_in_School_Activities","Neatness","Honesty","Self_Control","Relationship_with_others","Responsibility_Leadership","Games_Sports","Handling_of_Tools_Lab_and_Workshop","Drawing");
	$fetch=mysqli_query($con, "SELECT * FROM affective_trait WHERE student_id='$end' && term_id='$term1' && session='$sessionValue'")or die(mysqli_error());
	$dla= '<center><table width="60%" border="0" class="table table-hover table-responsive"><tr>';
